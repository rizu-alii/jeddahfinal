  ...
  return (
    <div style={{ backgroundColor: "var(--brand-bg)", color: "var(--brand-text)" }}>
      ...
      <section className="relative min-h-[55vh] flex items-center justify-center overflow-hidden">
        <img src={feature.image.url} alt={t(`${categoryKey}.${feature.slug}.title`)} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="grid gap-10 md:grid-cols-5">
          <div className="md:col-span-3">
            ...
            <ul className="mt-4 space-y-3">
              {(t(`${categoryKey}.${feature.slug}.bullets`, { returnObjects: true }) as string[]).map((bullet: string, i: number) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="mt-1 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: "var(--gradient-brand)" }}>✓</span>
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="mx-auto max-w-6xl px-4 py-16 md:py-20">
            ...
            <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((r: typeof feature) => (
                ...
                >
                  <img src={r.image.url} alt={t(`${categoryKey}.${r.slug}.title`)} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-110" />
