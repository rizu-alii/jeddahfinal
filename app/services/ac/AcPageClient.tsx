...
import { heroIndoorAc as heroIndoorAcAsset } from "@/lib/assets";
...
const features = SERVICE_CATEGORIES.ac.features;
  ...
  return (
    <div style={{ backgroundColor: "var(--brand-bg)", color: "var(--brand-text)" }}>
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        <Image src={heroIndoorAcAsset.url} alt={t("services.ac")} fill sizes="100vw" className="object-cover" priority />
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 py-20 text-center text-white">
          ...
          <h1 className="mt-6 text-4xl font-extrabold sm:text-5xl md:text-6xl">{t("services.ac")}</h1>
          <p className="mx-auto mt-5 max-w-2xl text-base text-white/90 sm:text-lg">
            {t("ourServices.ac.desc")}
          </p>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 md:py-20">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <Link
              key={f.slug}
              href={`/services/ac/${f.slug}`}
              className="relative overflow-hidden rounded-2xl shadow-md group h-80 block"
            >
              <Image src={f.image.url} alt={t(`acFeatures.${f.slug}.title`)} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition duration-500 group-hover:scale-110" />
              ...
              <div className="absolute left-4 right-4 bottom-4 flex items-center justify-between gap-3 rounded-xl bg-white/95 px-4 py-3 shadow-lg backdrop-blur">
                <h3 className="text-sm font-bold leading-tight" style={{ color: "var(--brand-primary)" }}>{t(`acFeatures.${f.slug}.title`)}</h3>
