...
import { stove1 as s1 } from "@/lib/assets";
...
const features = SERVICE_CATEGORIES["gas-stove"].features;
  ...
  return (
    <div style={{ backgroundColor: "var(--brand-bg)", color: "var(--brand-text)" }}>
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        <img src={s1.url} alt={t("services.stove")} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 py-20 text-center text-white">
          ...
          <h1 className="mt-6 text-4xl font-extrabold sm:text-5xl md:text-6xl">{t("services.stove")}</h1>
          <p className="mx-auto mt-5 max-w-2xl text-base text-white/90 sm:text-lg">
            {t("ourServices.stove.desc")}
          </p>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 md:py-20">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <Link
              key={f.slug}
              href={`/services/gas-stove/${f.slug}`}
              className="relative overflow-hidden rounded-2xl shadow-md group h-80 block"
            >
              <img src={f.image.url} alt={t(`stoveFeatures.${f.slug}.title`)} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-110" />
              ...
              <div className="absolute left-4 right-4 bottom-4 flex items-center justify-between gap-3 rounded-xl bg-white/95 px-4 py-3 shadow-lg backdrop-blur">
                <h3 className="text-sm font-bold leading-tight" style={{ color: "var(--brand-primary)" }}>{t(`stoveFeatures.${f.slug}.title`)}</h3>
